"""Reset UDF — restore a default persona to its shipped seed.
...
------
id : str
    The default persona's slug or derived id.

Returns
-------
dict
    The restored AgentRecord, or ``{"ok": false, "error": "<reason>"}`` (unknown
    agent, or "only default agents can be reset" for a custom persona).

Note: ``createdAt`` is derived from the file mtime (never persisted), so it is not
...
def reset(id: str = "", app_dir: str = "", seed_file: str = "") -> dict:
    ...
    seeds = _load_seeds()
    seed = next((s for s in seeds if s["slug"] == slug or _derive_id(s["slug"]) == id), None)
    if seed is None:
        if existing is None:
            return {"ok": False, "error": "not found"}
        return {"ok": False, "error": "only default agents can be reset"}
    record = _seed_record(seed)
    _write_agent_files(record)
    agent_file = os.path.join(_agents_dir(), record["slug"], AGENT_FILE)
    record["id"] = _derive_id(record["slug"])
    record["createdAt"] = _iso_from_mtime(agent_file)
    return record
