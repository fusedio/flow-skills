"""Delete-project UDF — remove every task (and its comments) for one project.
...
This is the task-management half of the cross-skill **project-delete** cascade:
each management skill deletes its OWN collection(s) for the named project, joined
by the camelCase ``project`` field (comments carry no ``project`` of their own, so
they join via ``taskId``). Unlike the task-tree ``delete(id)`` (which cascades a
single task's subtree across every collection), ``delete_project`` is one of three
independent per-project deleters — it touches ONLY ``tasks`` + ``comments`` and
leaves runs/cards/costEvents to their owners (run-management / feedback-management
/ Flow).

Params
------
project : str
    The project slug whose tasks (and their comments) are removed.
...
Idempotency
-----------
An empty/whitespace ``project`` — and any project with no matching tasks — is a
no-op that returns zero counts and writes nothing, so a re-run on an
already-deleted project never raises.
"""
...
def delete_project(project: str = "", app_dir: str = "") -> dict:
    """Remove every task for ``project`` and the comments on those tasks.

    One atomic ``_load_doc → mutate → _save_doc`` cycle over ``tasks`` +
    ``comments``. Idempotent: an empty/whitespace ``project`` — or one with no
    tasks — returns zero counts and writes nothing.

    Args:
        project: the project slug whose tasks (and their comments) are removed.
    ...
    if not project.strip():
        return {"deletedTaskIds": [], "tasksRemoved": 0, "commentsRemoved": 0}
