"""Cancel-task-cards UDF — sweep every pending card on a task to cancelled.
...
--------------------
task : str
    The task id whose pending cards are swept (matched against ``taskId``).

Returns
-------
list[dict]
    The cards this call cancelled, in ``cards[]`` order. An empty sweep returns
    ``[]`` and writes nothing.
"""
...
def cancel_task_cards(task: str = "") -> list:
    ...
    cancelled: list[dict] = []
    for card in cards:
        if card.get("taskId") != task or card.get("status") != "pending":
            continue
        card["status"] = "cancelled"
        card["resolvedBy"] = "user"
        card["resolvedAt"] = now
