"""Create UDF — add a new custom persona to the live app roster directory.

Writes ``<agents>/<slug>/AGENTS.md`` (+ the ``.openfused.yaml`` sidecar entry when
the persona carries non-default adapter/model) under ``~/.openfused/app/agents/``
(or ``$OPENFUSED_APP_DIR_STATE/agents``). Mirrors ``createAgent``: required-field
+ slug-collision rules, slug derived from the name, ``builtin: false``.
...
Params
------
name, title, role, description, prompt : str
    Required (non-empty) persona fields.
model : str
    Optional model override; empty string → no override (null).
effort : str
    Optional reasoning effort (one of low|medium|high|xhigh|max); empty or
    unknown → the ``"high"`` default.
adapter : str
    Optional adapter; empty string → the default (``claude_code``).
slug : str
    Optional explicit slug; empty string → derived from ``name``.

Returns
-------
dict
    The new AgentRecord, or ``{"ok": false, "error": "<reason>"}`` on a
    required-field / slug-collision failure.
"""
...
def create(
    ...
    Args:
        name, title, role, description, prompt: required persona fields.
        model: optional model override (empty → null).
        effort: optional reasoning effort (empty/unknown → "high").
    ...
    resolved_slug = (slug.strip() or _derive_slug(name)).lower()
    if not resolved_slug:
        return {"ok": False, "error": "name must contain at least one letter or digit"}
    _seed_default_roster()
    if any(a["slug"] == resolved_slug for a in _load_roster()):
        return {"ok": False, "error": f'an agent with slug "{resolved_slug}" already exists'}
    record = {
        "id": _derive_id(resolved_slug),
        "slug": resolved_slug,
        "name": name.strip(),
        "title": title.strip(),
        "role": role.strip(),
        "description": description.strip(),
        "adapter": adapter.strip() or DEFAULT_ADAPTER,
