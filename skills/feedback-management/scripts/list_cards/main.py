"""Read UDF — returns interaction-card records from the live app state file.
...
------
task : str
    Filter to a single task's cards (matched against the camelCase ``taskId``
    field). Empty string (the default) returns all cards across all tasks.

Returns
-------
list[dict]
    Raw camelCase ``InteractionCardRecord`` dicts, oldest-first by ``createdAt``
    (mirrors the Express ``listCards`` ordering in app/src/server/store/cards.ts).
"""
...
def list_cards(task: str = "") -> list:
    ...
    if task:
        cards = [c for c in cards if c.get("taskId") == task]
    return sorted(cards, key=lambda c: c.get("createdAt", ""))
