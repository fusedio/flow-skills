"""Create UDF — appends a new run record to the live app state file.
...
Mirrors ``createRun`` in ``app/src/server/store/runs.ts``.

Params
------
id : str
    The caller-supplied run id (``run_<hex>``). The app mints the id before
    persisting because it also keys an in-memory live buffer by it, so this UDF
    does NOT mint a new one.
task_id : str
    The task this run advances (stored as the camelCase ``taskId`` field).
prompt : str
    The composed prompt sent to the adapter.

Returns
-------
dict
    The newly created camelCase ``RunRecord`` (13 fields).
"""
...
def create(id: str = "", task_id: str = "", prompt: str = "") -> dict:
    ...
    Mirrors ``createRun(taskId, prompt, id)`` in
    ``app/src/server/store/runs.ts``: ``status`` starts ``"queued"`` (the run is
    minted before it launches — it may wait for a concurrency slot or a blocker;
    the launcher promotes it to ``"started"`` via ``mark_started`` the moment the
    agent process goes live), ``createdAt`` is stamped now, and every other field
    is ``null``.
