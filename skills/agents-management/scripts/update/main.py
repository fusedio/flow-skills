"""Update UDF — patch fields on an existing persona in the live app roster.
...
The Express ``updateAgent`` enforces the two rules that boundary can't express —
an explicitly-empty REQUIRED field is rejected with a 400 before delegating here,
and clearing ``model`` back to null arrives as the ``MODEL_CLEAR_SENTINEL`` value
(a bare empty string would be indistinguishable from "leave unchanged").

Returns
-------
dict
    The updated AgentRecord, or ``{"ok": false, "error": "not found"}``.
"""
