"""Set-prompt UDF — updates the stored prompt on a not-yet-finished run.
...
prompt : str
    The new composed prompt.

Returns
-------
dict
    The updated camelCase ``RunRecord`` on success, or
    ``{"ok": false, "error": "not found"}`` when the run is absent.
"""
