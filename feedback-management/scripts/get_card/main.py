"""Read UDF — returns a single interaction-card record from the live app state file.
...
------
id : str
    The ``card_<hex>`` id to look up. Empty string never matches.

Returns
-------
dict
    The raw camelCase ``InteractionCardRecord`` dict (mirrors
    app/src/server/store/cards.ts ``getCard``), or
    ``{"ok": False, "error": "not found"}`` when no card matches.
"""
