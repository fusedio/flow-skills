...
(`widget`/`udf`/`reference`) and `artifact_stem` further scope to one artifact when
non-empty. `get-one-by-id` is `SELECT * FROM {{read}} WHERE id = '...'`.
...
| `project` | str | |
| `artifactType` | str | `widget` / `udf` / `reference` |
| `artifactStem` | str | widget stem / udf name / reference name |
