...
``.openfused.yaml`` sidecar entry. Mirrors ``deleteAgent`` in
``app/src/server/store/roster.ts``. It does **not** touch the seed ledger, so a
deleted default stays deleted on the next seed pass (the ledger still lists it).
