"""Mark-started UDF — promote a queued run to live (queued → started).
...
Mirrors ``markRunStarted(id)`` in ``app/src/server/store/runs.ts``: a run is
minted ``"queued"`` by ``create`` and stays queued while it waits for a
concurrency slot / its blockers; the launcher calls this the moment the agent
process actually goes live to flip the persisted status to ``"started"``. This
is the NON-terminal transition the ``finish`` UDF cannot serve (``finish``
stamps ``finishedAt`` — a terminal-only write).

Params (all strings)
--------------------
id : str
    The run id (``run_<hex>``) to promote.

Returns
-------
dict
    The updated run record, or ``{"ok": False, "error": "not found"}`` when no
    run carries that id.
"""
...
def mark_started(id: str = "") -> dict:
    ...
    Idempotent — promoting an already-``"started"`` run is a no-op write.
