---
name: openfused-cli
description: Reference for the openfused CLI — environment management, file storage, secrets, code execution, and infrastructure commands. Use when writing or explaining shell commands that invoke `openfused`, or when helping users set up, switch between, or provision environments.
---

# openfused CLI reference

Check the installed version with `openfused --version` (useful for confirming an install before configuring anything).
...
```sh
openfused --env prod files list
openfused --env staging secrets list
```
...
```sh
openfused --backend aws files list        # reads OPENFUSED_* env vars
openfused --backend local files list      # host venvs (uv/pip)
openfused --backend fused files list      # Fused's managed openfused
```
...
4. Multiple environments, no pin → error naming both fixes: set `default_env` (`openfused project set <project> --env <name>`) or pass `--env`
5. No environments → error: run `openfused env create`
...
```sh
OPENFUSED_LOG_LEVEL=DEBUG openfused --env prod code run --file job.py
```
...
openfused env create prod --backend aws --prefix myapp- --region us-east-1

# AWS — skip provisioning (config only)
openfused env create staging --backend aws --prefix myapp-staging- --no-provision
...
```sh
openfused env list            # all envs with their backend
openfused env show prod       # JSON dump of config
...
```sh
openfused project set my-project --env prod   # validates env exists, writes default_env
openfused project set my-project --clear-env  # remove the pin
```

### Update fields

```sh
openfused env update prod --region us-east-1
openfused env update prod --prefix newprefix- --lambda-timeout 600
openfused env update prod --audit-bucket my-audit-bucket   # add/change audit bucket
openfused env update prod --no-audit-bucket                # remove audit bucket
openfused env update prod --require-spec                   # block executions without a spec
openfused env update prod --no-require-spec                # remove spec requirement
openfused env update prod -p pandas -p duckdb   # set packages (AWS only: baked into the container image)
```
...
```sh
openfused env delete staging --yes   # removes config only; does NOT teardown AWS resources
```
...
It runs code on **Fused's hosted, managed openfused** environment over its data-plane MCP endpoint (an MCP client of the remote openfused tool surface). The local side provisions nothing and runs no code itself. Note: `serve` and `infra` commands are not supported on Fused. Create an env with `--backend fused`:
...
| `--api-key-secret` | — | Name in openfused's local secrets store holding the `ofs_` API key. |

```bash
openfused env create fused-prod --backend fused --tier prod \
...
openfused fused redeem CODE [--tier prod]       # redeem a beta invite: admit + create your org + env
openfused fused orgs [--tier prod]              # list your orgs + envs and their provision_state
openfused fused setup [--tier prod] \           # the one-shot guided flow:
...
openfused fused key create --org O --env E      # mint + store a key for an existing managed env
openfused fused key revoke --org O --id K       # revoke a data-plane key by id
openfused fused logout [--no-browser]           # delete the cached control-plane JWT
openfused fused logout --env NAME               # ALSO delete that env's stored data-plane key (full logout)
```

`setup` stores the minted key in the local secrets store (e.g. `fused/<env-name>-key`) and writes a `FusedCloudEnvironmentConfig` referencing it — never the raw key in `envs.json`. The openfused env name defaults to `fused` for the canonical `default` managed env (else `fused-<env>`). The control-plane JWT is flow-scoped; the MCP server never holds it.

Token resolution at request time (first hit wins): **`FUSED_API_KEY`** env var (an explicit override) → the stored **`api_key_secret`** → **`FUSED_JWT`** (scoped-URL only). A configured-but-absent secret falls through rather than failing. **Logging out:** `openfused fused logout` clears the control-plane JWT; add `--env NAME` to also delete that environment's stored data-plane key (then `fused key revoke` to revoke it server-side).
...
openfused env update prod --verify '{"enabled": true, "typecheck": true}'

# Also run ty inside Docker so user packages are installed (more accurate)
openfused env update prod --verify '{"enabled": true, "typecheck": true, "typecheck_docker": true}'
```
...
```sh
openfused project new taxi-pipeline
...
```sh
openfused project add-dep taxi-pipeline duckdb pandas        # runtime deps
openfused project add-dep taxi-pipeline pytest coverage --dev # dev deps (for `code test`)
```
...
```sh
openfused project show taxi-pipeline
```
...
│   └── scripts/
│       ├── pyproject.toml          # uv-managed Python deps (note: uv's [project] table, not openfused's)
...
3. Validate with `openfused code verify <file>` (CLI) or MCP `verify_code` before committing.
...
```sh
openfused project deploy taxi-pipeline                     # deploy all UDFs to preview
openfused project deploy taxi-pipeline --channel release   # deploy all UDFs to release
openfused project deploy taxi-pipeline --force             # bypass dirty-tree check
```
...
Requires AWS env + `cache_bucket` + provisioned serving plane (`openfused infra serve`). Echoes the resolved env name; prints one URL per UDF. Exits 1 if any UDF fails to deploy.
...
```sh
openfused project status taxi-pipeline
```
...
```sh
openfused udf deploy analysis --project taxi-pipeline
openfused udf deploy analysis --project taxi-pipeline --channel release
openfused udf deploy analysis --project taxi-pipeline --force
```
...
```sh
openfused udf promote analysis --project taxi-pipeline
```
...
```sh
openfused udf rollback analysis --project taxi-pipeline
openfused udf rollback analysis --project taxi-pipeline --to abc123def
```
...
```sh
openfused udf retire analysis --project taxi-pipeline
openfused udf retire analysis --project taxi-pipeline --yes
openfused udf retire analysis --project taxi-pipeline --yes --force   # take over a foreign-owned UDF
```
...
The hook blocks manual commits that touch a UDF's `spec.md` without its entrypoint (`main.py`/`main.json`), or vice versa — including one-sided deletions. The pairing is enforced at depth 4: `<project>/scripts/<udf>/<file>`. Tests, resource files, and other files commit freely. openfused's own auto-commits are always paired and pass through without `--no-verify`.
...
```sh
openfused files list                        # list all buckets
openfused files list --bucket my-bucket     # list all keys in bucket
openfused files list --bucket my-bucket --prefix data/2024/
```

### Count

```sh
openfused files count --bucket my-bucket
openfused files count --bucket my-bucket --prefix logs/ --ext .parquet --ext .csv
```

### Get presigned URL

```sh
openfused files get --bucket my-bucket --key data/report.parquet
openfused files get --bucket my-bucket --key data/report.parquet --expires-in 7200
```
...
```sh
openfused files schema --bucket my-bucket --key data/report.parquet
```

### Upload

```sh
openfused files upload data.parquet --bucket my-bucket --key uploads/data.parquet
cat data.csv | openfused files upload - --bucket my-bucket --key uploads/data.csv
```
...
```sh
openfused inloop                 # serve the built app at http://127.0.0.1:4400 + open the browser
openfused inloop --port 5000
openfused inloop --dev           # vite + tsx watch (development)
openfused inloop --no-open       # don't open the browser (default: --open)
```
...
The app **ships inside the installed package**, so `pip install openfused` is
enough to run `openfused inloop` — no source checkout, no `node_modules`. The only
...
`openfused doctor` surveys **every** workspace under `~/.openfused/workspaces/`
...
openfused doctor --fix    # remediate the fixable findings, then re-diagnose
```
...
```sh
openfused project create taxi --description "NYC taxi analysis"   # scaffold under workspaces/default/taxi
openfused project create taxi --env prod-aws                      # scaffold and pin default_env (validates env exists)
openfused project list                       # registered projects (name + path + exists flags)
openfused project show [NAME]                # the context packet (same as get_project_context)
openfused project set NAME --description "new words" --env prod   # update the manifest's [project] keys
openfused project set NAME --clear-env       # remove default_env from the manifest
openfused project serve --mcp --project NAME # serve ONE project as a READ-ONLY stdio MCP (the product tier)
```
...
Start agent work in a project with `openfused project show` (or the
...
```sh
openfused project serve --mcp --project taxi-analysis   # read-only stdio MCP for one project
openfused code serve ./taxi --mcp                       # equivalent: serve the project DIR read-only
```
...
`openfused` stdio server (run with no subcommand), which registers the full *gated*
...
openfused pipeline graph --project taxi          # explicit project
openfused pipeline graph --canvas pipelines/reporting/canvas.toml  # a named canvas
openfused pipeline derive                        # "create canvas": write canvas.toml + emit reloaded graph
openfused pipeline derive --project taxi          # explicit project
```
...
```sh
openfused audit log                          # last 50 events
openfused audit log --limit 100
openfused audit log --status blocked         # blocked executions only
openfused audit log --event-type execute_code --status warned
openfused audit log --project taxi           # events recorded under one project
```
...
```sh
openfused secrets put db-password "s3cr3t"       # create or update
openfused secrets get db-password                # print value
openfused secrets list                           # all secrets
openfused secrets list --prefix db-             # filter by prefix
openfused secrets delete db-password             # delete (prompts; --yes to skip)
```
...
```sh
openfused secrets put openfused-db-password "s3cr3t"   # readable from Lambda
...
openfused code run myanalysis.py

# From stdin
cat myanalysis.py | openfused code run

# Pass local files into the execution context
openfused code run myanalysis.py --input-file data.parquet --input-file config.json
```

pip requirements are configured per-environment via `env update -p`, not per-call. Set them once:

```sh
openfused env update prod -p pandas -p duckdb
```
...
**Keep the package set stable across calls.** On AWS, packages are baked into the container image — changing them means rerunning `openfused infra build-image` (build + ECR push). On local, venvs are cached by a hash of the package set — changing it rebuilds the venv (seconds with uv).
...
```sh
# Scan a file
openfused code verify myanalysis.py

# Inline scan
openfused code verify -c "import subprocess; result = 1"

# Scan code + input files for PII and path traversal
openfused code verify myanalysis.py --input-file data.csv

# Spec check — Claude reviews whether code matches description (requires an Anthropic
# API key: ANTHROPIC_API_KEY env var, or `openfused secrets put anthropic-api-key ...`)
openfused code verify myanalysis.py --spec "compute the mean of column A"

# Scan using a workspace project's deps
openfused code verify myanalysis.py --project taxi-pipeline
...
openfused code test mymodule.py --test-file test_mymodule.py --input-file data.csv
```
...
**Local backend — `--project` or `--project-dir` is required.** On a local environment, `code test` requires one of these flags. pytest and coverage must be declared as dev dependencies in the project's pyproject.toml; they are not auto-installed in the project venv. Add them in one step with `openfused project add-dep <project> pytest coverage --dev` (runs `uv add --dev` + `uv sync`, so the venv isn't left stale). A stale project venv is auto-reconciled on `code run`/`code test` before executing (set `OPENFUSED_NO_VENV_SYNC=1` to opt out).

```sh
# Workspace-registered project
openfused code test mymodule.py --test-file test_mymodule.py --project taxi-pipeline
...
openfused code serve myudf.py --port 8000

# Inline code — requires -c/--code → GET+POST /run
openfused code serve -c "result = 1 + 1"

# From stdin with custom route name → GET+POST /tiles
cat myudf.py | openfused code serve --name tiles

# Override route name for a file → GET+POST /api
openfused code serve myudf.py --name api --port 8000

# With static files available in every request's execution context
openfused code serve myudf.py --input-file model.pkl --input-file config.json

# Bind publicly
openfused code serve myudf.py --host 0.0.0.0 --port 8080
...
#  `openfused project migrate` — it is not served as-is.)
openfused code serve ./my_project   # → GET+POST /daily, /stars, …

# Serve a project directory as the READ-ONLY MCP product tier (stdio, not HTTP)
openfused code serve ./taxi --mcp   # equivalent to `project serve --mcp` for that project
```
...
- **`openfused infra serve`** provisions the environment's serving plane (one
  HTTP API + one dispatcher Lambda) — see *Infrastructure* below;
- **`openfused share create`** publishes an app and mints its URL with the access
  control you choose — see the `share` section below;
- **`openfused share revoke`** / **`infra serve --rate-limit 0`** /
...
openfused share create my_file.py
...
openfused share create --public my_file.py

# Named public mount — deliberately guessable; requires the explicit --token
# (prints a warning)
openfused share create --public --token acme-dash my_file.py

# Custom issuer (must be on the env's serve_auth.issuer_allowlist) / ACL
openfused share create --jwt-issuer https://idp.acme.io/ --jwt-audience proj my_file.py
openfused share create --acl-subject alice@acme.io --acl-group analysts my_file.py

# Allow a browser origin to call the mount (repeatable for multiple origins)
openfused share create --public --cors-origin https://app.acme.io my_file.py
...
openfused share create ./project
openfused share create ./project --entrypoint daily --not-after 2026-12-31T00:00:00Z

# Inspect (list shows YOUR mounts; --all for everyone's)
openfused share list
openfused share list --all
openfused share show my-file
...
openfused share revoke my-file
openfused share revoke my-file --confirm
...
openfused share recreate my-file
openfused share recreate my-file --same-token
...
openfused share repoint my-file new-code.py
openfused share repoint my-file new-code.py --entrypoint daily
openfused share repoint my-file new-code.py --public
openfused share repoint my-file new-code.py --jwt-audience new-proj
...
openfused share repoint my-file new-code.py --confirm
```
...
```sh
openfused infra plan        # dry run — exits 1 if changes needed
openfused infra apply       # reconcile IAM role, Lambda functions to desired state
openfused infra teardown    # delete all openfused Lambdas + IAM role (prompts for confirmation)
openfused infra teardown --yes   # skip prompt
```
...
```sh
openfused infra plan          # reports missing dirs (exits 1 on drift)
openfused infra apply         # create dirs (idempotent; bare venv is created lazily on first execute)
openfused infra teardown      # remove the venvs dir + data dir (prompts); serve endpoints survive
openfused infra lambda-reset  # clear the in-process venv ready-cache (nothing deleted from disk)
```
...
Docker to build the dispatcher image (the openfused package on the Lambda Python
base; `--image-uri` registers a pre-built image instead). Idempotent.

```sh
# Provision (or reconcile) the plane
openfused infra serve
...
openfused infra serve --rate-limit 50
openfused infra serve --rate-limit 0

# Custom domain (every mount is hosted under it by path)
openfused infra serve --domain api.example.com --cert-arn arn:aws:acm:...
...
openfused infra serve --teardown --yes
```
...
```sh
openfused infra build-image                               # builds in CodeBuild (default); streams logs
openfused env update prod --context-dir ./img --dockerfile Prod.Dockerfile  # your own Dockerfile + context
openfused infra build-image --builder local -p duckdb    # opt into a host docker build
```
...
- **Project-ignorant.** Project targets (`widget open {project,stem}`, project parley
  pushes) are **delegated to `openfused dev serve`'s `?workspace=&project=`
...
- **Own `dev serve` child.** The widget-host spawns its **own** headless `openfused
  dev serve` daemon (separate from the one the `openfused up` app spawns) and resolves
...
```sh
openfused widget open sales_overview                  # a saved widget (stem of ./sales_overview.json)
openfused widget open ./draft.json --title "Draft"    # any config file → temporary /_tmp/<id> path
openfused widget open sales_overview --timeout 0      # wait forever
openfused widget open sales_overview --stream         # NDJSON: every event as it happens
```

| Option | Default | Notes |
|---|---|---|
| `--project NAME` | — | Project owning a saved-widget stem TARGET (default: resolved project). Mutually exclusive with `--project-dir`. |
| `--project-dir PATH` | — | Pin the resolve daemon to a project directory (`.json` targets only). Drives `openfused dev serve`'s `?projectDir=<PATH>` mode (UDFs from `<PATH>/scripts/`, run under `<PATH>/scripts/.venv`) so the project's UDFs and `.venv` are available. Mutually exclusive with `--project`. |
...
```sh
openfused widget push sales_overview        # push a saved widget into the parley page
openfused widget push ./draft.json --title "Draft"   # any config file works too
openfused widget watch                      # stream the human's actions (NDJSON, runs until stopped)
openfused widget watch --verbose            # ...also every per-input params change (noisy)
openfused widget parley                     # print/open the parley page URL
```
...
`openfused dev serve` is the **single widget-data serving daemon** — it executes
...
`openfused dev serve` starts a single long-running loopback HTTP server that
...
```sh
openfused dev serve                              # ephemeral port, runs until stopped
openfused dev serve --host 127.0.0.1 --port 9100
openfused dev serve --timeout 300               # auto-stop after 5 minutes
```
...
```sh
openfused env create prod --backend aws --prefix myapp- -p pandas
# Provisions IAM role + cache bucket
openfused infra build-image   # build + push the Lambda container image (required before execution)
openfused infra apply         # creates the {prefix}container Lambda from the image
openfused infra plan          # verify no further drift
```

### Targeting a specific environment for a single command

```sh
openfused --env staging secrets list   # uses staging for this command only
```

### CI drift check

```sh
openfused infra plan || echo "Infrastructure out of sync — run apply"
```

### Inspecting a dataset before running expensive code

```sh
openfused files schema --bucket my-bucket --key data/large.parquet
openfused files count --bucket my-bucket --prefix data/ --ext .parquet
```
