"""Update-status UDF — sets the status field on a task record.
...
status : str
    The new status value (e.g. ``pending``, ``todo``, ``in_progress``,
...
def update_status(id: str = "", status: str = "") -> dict:
    ...
    Mirrors ``TasksStore.update_task_status`` in ``tasks.py``.  The real method
    raises ``TaskNotFoundError`` on a missing task; here we return an informative
    ack dict instead (no exceptions across the UDF boundary).
