"""Resolve UDF — the single guarded pending → terminal card transition.
...
--------------------
id : str
    The ``card_<hex>`` id to resolve.
status : str
    The terminal status the route computed (never ``pending``):
    ``answered`` / ``superseded`` / ``cancelled``.
result : str
    The generic ``{action, params}`` result, JSON-encoded; empty string → null.
resolved_by : str
    Who resolved it (the route passes ``"user"``); empty string → null.

Returns
-------
dict
    The resolved card record on success, or
    ``{"ok": False, "error": "not found"}`` (unknown id) /
    ``{"ok": False, "error": "already resolved"}`` (non-pending) on a miss.
"""
...
def resolve_card(
    ...
    Args:
        id: the card id to resolve.
        status: the terminal status the route computed, one of
            ``answered`` / ``superseded`` / ``cancelled`` (never ``pending``).
    ...
    if result:
        try:
            ...
            raise ValueError(f"resolve_card `result` is not valid JSON: {exc}") from exc

    doc = _load_doc("cards")
    cards: list[dict] = doc.get("cards") or []

    card = next((c for c in cards if c.get("id") == id), None)
    if card is None:
    ...
    card["status"] = status
    card["result"] = result_obj
    card["resolvedBy"] = resolved_by or None
    card["resolvedAt"] = now
