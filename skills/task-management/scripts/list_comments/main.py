"""list_comments UDF — returns comment records for a task from the live app state file.
...
------
task_id : str
    The task id to filter on. Empty string (the default) returns [].

Returns
-------
list[dict]
    Raw camelCase comment records for the given task, oldest-first by ``createdAt``.
    Empty list if task_id is empty or no comments match.
"""
...
def list_comments(task_id: str = "", app_dir: str = "") -> list:
    ...
    matching = [c for c in comments if c.get("taskId") == task_id]
    return sorted(matching, key=lambda c: c.get("createdAt", ""))
