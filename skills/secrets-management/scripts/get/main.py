"""Get UDF — reveals a single secret's cleartext value from the OS keychain store.
...
Returning cleartext through the dev-serve ``{"data": ...}`` envelope is accepted for
this surface (localhost + token, the same posture as the app's lazy-reveal route).

Params
------
name : str
    The secret name to reveal.

Returns
-------
dict
    ``{"name": <name>, "value": <cleartext>}`` on success;
    ``{"ok": False, "error": "not found"}`` when the secret is absent (an ack rather
    than a raised ``KeyError`` — mirrors the task UDFs' missing-record convention).
"""
