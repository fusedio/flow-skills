"""Clone UDF — create a custom (``builtin: false``) editable copy of any persona.
...
------
id : str
    The source persona's slug or derived id.
name : str
    The new persona's name (required; slug derived from it).

Returns
-------
dict
    The new AgentRecord, or ``{"ok": false, "error": "<reason>"}`` (source not
    found, missing name, or slug collision).
"""
...
def clone(id: str = "", name: str = "") -> dict:
    ...
    source = next((a for a in _load_roster() if a["slug"] == id or a["id"] == id), None)
    if source is None:
        return {"ok": False, "error": "not found"}
    resolved_slug = _derive_slug(name)
