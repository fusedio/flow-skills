...
Mirrors run-management/set_prompt/main.py: stdlib-only, takes the exclusive flock
on the `artifactChats` collection, whole-document read-modify-write (preserves
every other top-level key), atomic write.
...
def set_title(chat_id: str = "", title: str = "", app_dir: str = "") -> dict:
    """Set the stored title on a chat unconditionally; empty → null.

    Mirrors ``set_prompt`` (a missing record returns an informative ack). No
    state-machine validation — the app gates legality before calling.

    Args:
        chat_id: the chat id to label.
        title: the new title; empty string → null.
